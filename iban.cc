...
bool Validate::isValid(std::string arg) {
	...
	if (!(this->selectedSpec == nullptr)) {
		...
		return this->selectedSpec->length == spec->length
		       && this->selectedSpec->countryCode.compare(
		           spec->countryCode) == 0
		       && std::regex_match(shortened, parseStructure(
		                               this->selectedSpec->structure))
...
Validate::Validate() {
	addSpecification(std::move(std::make_unique<Specification>(
	                               "AD", 24, "F04F04A12",
